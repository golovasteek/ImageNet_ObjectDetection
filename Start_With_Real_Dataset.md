Схема обучения нейронной сети в Tensorflow
------------------------------------------
...
[методом обратного распространения ошибки][back_propagation].
Нейронная сеть в Tensorflow представляется последовательностю операций-слоев
(таких как перемножение матриц, [свертка][conv_nn], пулинг и т.д.). Слои нейронной сети совместно
...
![Predicticting graph][alex_net_graph]
с графом который в том числе выполняет подстройку коэффициенотов
![Training graph][alex_net_training_graph].
...
[back_propagation]: https://ru.wikipedia.org/wiki/Метод_обратного_распространения_ошибки
[conv_nn]: https://ru.wikipedia.org/wiki/Свёрточная_нейронная_сеть
[alex_net_graph]: Pictures/alexnet.png
[alex_net_training_graph]: Pictures/alexnet_train.png